...
| 이름 | GitHub | 역할 |
|---|---|---|
| 주성민 | iconflorinity | 기획,프론트엔드,백엔드 |
| 허서준 | gjtjwns06-ui | 기획,프론트엔드,백엔드 |
...
### Keep
- 문서 작업을 탄탄하게 해두어서 롤백할 필요 없이 개발에 집중할 수 있었다.
- AI 사용 시 바로 코드 작성 및 작업 요청하기 보다 작업 계획을 구상하라고 한 후, 해당 계획에 따라 작업을 수행하라고 하니 체감상 성능이 더 좋았다

### Problem
- 코드 리뷰를 제대로 하지 못해, 세부적인 코드 내용을 전부 이해하지 못해 아쉬웠다.
- 부가 기능들을 추가할 때, DB 스키마를 변경해야 해서 번거로웠다.

### Try
- 이틀 차에 문서 작업을 하면서 개발 환경 세팅도 틈틈히 해두자.
- merge는 팀원과 상의되는 대로 바로바로 하자.
