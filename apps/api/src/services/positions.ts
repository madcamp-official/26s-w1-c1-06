 ...
 * 개별 투자자 신원은 감추고 매수/공매도 인원·수량 합계만 공개해 사회적 긴장감을 만든다.
 * 종목 본인(자기 약속에 걸린 베팅 구경) 또는 그 종목의 친구만 조회할 수 있다(베팅 가능 범위와 동일).
 */
export async function getBettorSummary(
  ...
  if (viewerId !== stockUserId) {
    ...
    if (!isFriend) {
      throw new HttpError(403, "친구인 종목의 베팅 현황만 조회할 수 있습니다.");
    }
  }
